...
module qmult #(
    ...
    (* use_dsp = "no" *)
    reg [2*N-1:0]	f_result;		//	Multiplication by 2 values of N bits requires a 
     //(*dont_touch = "yes"*) reg fast;		//	Multiplication by 2 values of N bits requires a 
     (*dont_touch = "yes"*) reg [2*N-1:0] fast;    
    (*dont_touch = "yes"*) reg normal;                              //	register that is N+N = 2N deep
    ...
    always @(posedge clk) begin 
       (* NATIVE_IMPLEMENTATION = "LUT" *)
       f_result <= multiplicand[N-2:0] * multiplier[N-2:0];  // We remove the sign bit for multiplication
     end
     
     always @(posedge fast_clk) begin
        begin 
          (* NATIVE_IMPLEMENTATION = "LUT" *)
          fast <=  multiplicand[N-2:0] * multiplier[N-2:0]; // We remove the sign bit for multiplication
        end  
     end
